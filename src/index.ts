...
export type SlimArray = any[] & SlimReserveFlags;
...
export function isSlimArray(x: any): x is SlimArray {
  return isArray(x);
}

export const ARRAY_OPERATION_EXTRACT = ["shift", "pop"] as const;
export const ARRAY_OPERATION_ADD = ["push", "unshift"] as const;
export const ARRAY_OPERATION = [...ARRAY_OPERATION_EXTRACT, ...ARRAY_OPERATION_ADD, "splice", "reverse"] as const;
export type ArrayOperation = typeof ARRAY_OPERATION[number];
...
export function isArrayChange(target, property: string | number) {
...
const _getProperty = function (obj, path: string) {
 ...
 */
const _create = function (target, domDelay: number | boolean, originalObservable?, originalPath?: PathMeta[]) {
  ...
  const _getPath = function (target, property) {
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      if (lastTarget instanceof Array && !isNaN(property as number)) {
    ...
    get: function (target, property) {
      ...
      if (isSlimArray(target) && ARRAY_OPERATION.includes(property)) {
        if (target.__slim_arrOpPending) {
          ...
          if (
            !(ARRAY_OPERATION_EXTRACT.includes(property) && target.length === 0) &&
            ...
            Promise.resolve().then(() => {
              ...
              function isSame() {
                return (
                  OperationTypeEnum.SORT === operationType && JSON.stringify(newValue) === JSON.stringify(oldValue)
                );
              }
 ...
 */
export const observe = function (proxy, observer) {
 ...
 * @returns {void} Does not return any value.
 */
export const pause = function (proxy) {
  var i = observables.length;
 ...
 * @returns {void} Does not return any value.
 */
export const resume = function (proxy) {
  var i = observables.length;
  var foundMatch = false;
  while (i--) {
    if (observables[i].parentProxy === proxy) {
 ...
 * @returns {void} Does not return any value.
 */
export const pauseChanges = function (proxy) {
  var i = observables.length;
  var foundMatch = false;
  while (i--) {
    if (observables[i].parentProxy === proxy) {
 ...
 * @returns {void} Does not return any value.
 */
export const resumeChanges = function (proxy) {
  var i = observables.length;
  var foundMatch = false;
  while (i--) {
    if (observables[i].parentProxy === proxy) {
 ...
 * @returns {void} Does not return any value.
 */
export const remove = function (proxy) {
